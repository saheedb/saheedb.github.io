...
### Alternate method to generate and save the images using arrays instead of nested loops:    
...
step_fn <- function(s_0) {
  ...
  for (cell in i_cell:z_cell) {
    border_cells <- cell + c(1, 9:11, -1, -9:-11)
    live_border  <- sum(s_0[border_cells] > 0)
    if (s_0[cell] == 1) {
      if (live_border %in% c(2, 3)) {
      } else {
        s_1[cell] <- 0
      }
    } else if (s_0[cell] == 0) {
...
## initial state represented by an array instead of a matrix
steps[[1]] <- rbinom(cols ^ 2, 1, 0.5)
for (i in 2:gens) {
  ## main function returns permuted array
